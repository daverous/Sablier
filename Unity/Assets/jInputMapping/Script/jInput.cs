...
public class jInput : ScriptableObject
{
		...
		/*SyncValuesData.csのScriptableObjectを取得し変数SyncValuesSODataに取る(ScriptableObjectgaが無ければ作成)
		#if (UNITY_EDITOR)
		void SyncValuesSOSetting ()
		{
				...
				if (Pathes.Length >= 1) {
						...
						SyncValuesData SyncValuesSOData = Resources.LoadAssetAtPath<SyncValuesData> (Path);
				} else if (Pathes.Length < 1) {
		...
		*/
		...
		static void InputNameUnsuitable ()
		{
				...
				GameObject.Find ("jInputMappingManager").GetComponent<Mapper> ().SaveFileDelete ();
		}
